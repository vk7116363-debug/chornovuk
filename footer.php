...
function openUniversalPopup(card) {
  ...
  const preorderText = card.dataset.preorderText || "";
    ...
    sizeItems.forEach(item => {
      ...
      if (!isDisabled) {
        el.addEventListener("click", () => {
         ...
         const modelInput =
        document.querySelector('.modern-form input[name="model"]');
...
const popupPreorderBox = document.getElementById("popupPreorderBox");

if (popupPreorderBox) {

  popupPreorderBox.innerHTML = "";
  popupPreorderBox.style.display = "none";

  if (preorder) {

    popupPreorderBox.innerHTML = `
      <div class="popup-preorder-wrap">

        <button type="button" class="popup-preorder-btn">
          💖 Забронювати з наступної поставки
        </button>

        <div class="popup-preorder-text">
          ${preorderText}
        </div>
    ...
    popupPreorderBox.style.display = "block";

    popupPreorderBox
      .querySelector(".popup-preorder-btn")
      .addEventListener("click", () => {

        const userSizeInput =
          document.getElementById(FORM_USER_SIZE_ID);

         const modelInput =
  document.querySelector('.modern-form input[name="model"]');

console.log("MODEL =", card.dataset.model);
...
@keyframes preorderPulse{
  ...
  0%{
    transform:scale(1);
    box-shadow:0 0 0 rgba(255,92,168,0);
  }

  50%{
    transform:scale(1.02);
    box-shadow:0 0 18px rgba(255,92,168,.45);
  }
...
<div class="reviews-more-block">
    <h3>💬 Щодня отримуємо нові фото та відгуки від наших покупців</h3>

    <a href="https://instagram.com/mriya_jeans" target="_blank" class="reviews-inst-btn">
        📸 Дивитися більше фото покупців в Instagram
    </a>
</div>
<style>
.reviews-inst-btn{
...
.reviews-inst-btn:hover{
    transform: translateY(-3px);
    box-shadow: 0 12px 28px rgba(221,42,123,.45);
}
